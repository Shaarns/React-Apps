...
const Todo = () => {
  ...
  //   setShowTodo(status)
  // }
  const buttonText = todo.length === 0 ? '' : 'Tasks Left '
  const completedText =
    todo.filter((item) => item.complete).length === 0 ? '' : 'Tasks Complete'
      ...
      <Container className='todoAppContainer'>
        <Grid container justify='space-around' spacing={1}>
          <Grid item md={5} xs={12}>
            ...
            <Typography className='taskLeft' variant='h6'>
              {buttonText}
              {buttonText && todo.filter((todo) => !todo.complete).length}
            </Typography>
          ...
          <Grid item md={5} xs={12}>
            ...
            >
              {completedText}{' '}
              {completedText && todo.filter((todo) => todo.complete).length}
            </Typography>
